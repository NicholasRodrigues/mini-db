...
func (p *Persistence) Save(data map[string]string) error {
	...
	log.Println("Saving data to Parquet file:", data)
	...
	for key, value := range data {
		log.Printf("Writing key: %s, value: %s\n", key, value)
...
func (p *Persistence) Load() (map[string]string, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			log.Println("Parquet file does not exist, returning empty data.")
			return data, nil
		}
	...
	log.Println("Loading data from Parquet file.")
	for {
		kv := make([]KeyValue, 1)
		...
		if len(kv) > 0 {
			log.Printf("Read key: %s, value: %s\n", kv[0].Key, kv[0].Value)
			data[kv[0].Key] = kv[0].Value
		}
	}

	log.Println("Data successfully loaded from Parquet file:", data)
	return data, nil
}
